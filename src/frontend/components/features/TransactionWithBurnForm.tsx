...
export default function TransactionWithForm({ provider, transferFlag }: Props) {
  const {
    register,
    handleSubmit,
    resetField,
  ...
  const onSubmitTransfer: SubmitHandler<Inputs> = async ({
    ...
    try {
      ...
      transferFlag(true);
      const tx = await contract.transferWithAutoBurn(
        recipient,
  ...
  return (
    <Box marginTop={10} width={"md"}>
      <Heading marginBottom={9}>Transfer With Burn</Heading>
      <form onSubmit={handleSubmit(onSubmitTransfer)}>
        <FormControl isInvalid={!!errors.amount || !!errors.recipient}>
          ...
          <Input
            ...
            borderColor={colorMode == "light" ? "blue.900 " : "teal.200"}
            {...register("recipient", {
          ...
          <Input
            ...
            borderColor={colorMode == "light" ? "blue.900 " : "teal.200"}
            {...register("amount", {
              required: "This is required. Minimum amount 1 AUT",
              min: {
                value: 1,
